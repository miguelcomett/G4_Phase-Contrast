...
//
//
// $Id: PepiPSPixiRad.cc 81087 2014-05-20 15:44:27Z gcosmo $
//
...
{ 
 ...
 // G4cout << "\taccumulated " << edep2 << G4endl;
  ...
  {
    G4double newval = 0;
    // energy blurring calibration from Laboratory Universidad de los Andes
//    G4double radThreshold1 = CLHEP::RandGauss::shoot(fThreshold+4*keV, (6.59+0.0728*(*(itr->second)/keV))/2.35*keV);
    G4double radThreshold = CLHEP::RandGauss::shoot(fThreshold, (6.59+0.0728*(*(itr->second)/keV))/2.35*keV);
    G4int i=0;
    // if the pixel index belong to the cluster it assigns all the signal to the first pixel to be hit (fIndex)
    if ((itr->first) == fIndex || (itr->first) == idx2 || (itr->first) == idx3 || (itr->first) == idx4)
    {
//       if(edep2 <= radThreshold1/keV &&  edep2 >= radThreshold/keV && i<1)
    ...
    {    
//        if (*(itr->second) <= radThreshold1 && *(itr->second) >= radThreshold)
