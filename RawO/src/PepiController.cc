...
//  PepiEventAction* pepiEventAction = (PepiEventAction*)(G4RunManager::GetRunManager()->GetUserRunAction());
  PepiRunAction* pepiRunAction = (PepiRunAction*)(G4RunManager::GetRunManager()->GetUserRunAction());
  
//  G4double runDose = pepiEventAction->GetObjectDose(fDoseType);
//  fDose += runDose; 
  ...
  std::vector<G4THitsMap<G4double>*> runMaps = pepiRunAction->GetHCofThisRun();
  
  G4double Dose = pepiRunAction->GetObjectDose();
  fDose = Dose;
  ...
  for (G4int i = 0; i < nMaps; i++)
  {
    ...
    if (runMap)
    {
      ...
      else if (runMap->GetName() == "Threshold3")
      {
        SaveData(runMap);
      }
      else if (runMap->GetName() == "Threshold4")
      {
        SaveData(runMap);
      }
      else if (runMap->GetName() == "Threshold5")
      {
        SaveData(runMap);
      }
      else if (runMap->GetName() == "Threshold6")
      {
        SaveData(runMap);
      }
      else if (runMap->GetName() == "Threshold7")
      {
        SaveData(runMap);
      }
      else if (runMap->GetName() == "Threshold8")
      {
        SaveData(runMap);
      }
...
//  fAction->GetObjectDose(Dose);
  ...
  G4double th3 = fDetector->GetThreshold3();
  G4double th4 = fDetector->GetThreshold4();
  G4double th5 = fDetector->GetThreshold5();
  G4double th6 = fDetector->GetThreshold6();
  G4double th7 = fDetector->GetThreshold7();
  G4double th8 = fDetector->GetThreshold8();
  
  std::string th1_str, th2_str, th3_str, th4_str, th5_str, th6_str, th7_str, th8_str ;
  std::ostringstream convert1, convert2, convert3, convert4, convert5, convert6, convert7, convert8 ;
  convert1 << std::setfill('0') << std::setw(5) << th1/eV;
  th1_str = convert1.str();
  convert2 << std::setfill('0') << std::setw(5) << th2/eV;
  th2_str = convert2.str();
  convert3 << std::setfill('0') << std::setw(5) << th3/eV;
  th3_str = convert3.str();
  convert4 << std::setfill('0') << std::setw(5) << th4/eV;
  th4_str = convert4.str();
  convert5 << std::setfill('0') << std::setw(5) << th5/eV;
  th5_str = convert5.str();
  convert6 << std::setfill('0') << std::setw(5) << th6/eV;
  th6_str = convert6.str();
  convert7 << std::setfill('0') << std::setw(5) << th7/eV;
  th7_str = convert7.str();
  convert8 << std::setfill('0') << std::setw(5) << th8/eV;
  th8_str = convert8.str();
...
if(type=="1COL" || type=="2COL" || type=="3COL" || type=="4COL" || type=="5COL" || type=="6COL" || type=="7COL" || type=="8COL" )
{
           fileName = fileName                                        + sep +            
                      "th1_"  +  th1_str + "eV";          		       
}
if(type=="2COL" || type=="3COL" || type=="4COL" || type=="5COL" || type=="6COL" || type=="7COL" || type=="8COL" )
{
           fileName = fileName                                        + sep +	      
                      "th2_"  + th2_str  + "eV";		       
}	       
if(type=="3COL" || type=="4COL" || type=="5COL" || type=="6COL" || type=="7COL" || type=="8COL" )
{
           fileName = fileName                                        + sep +
                      "th3_"  + th3_str  + "eV";
}
if(type=="4COL" || type=="5COL" || type=="6COL" || type=="7COL" || type=="8COL" )
{
           fileName = fileName                                        + sep +
                      "th4_"  + th4_str  + "eV";
}
if(type=="5COL" || type=="6COL" || type=="7COL" || type=="8COL" )
{
           fileName = fileName                                        + sep +
                      "th5_"  + th5_str  + "eV";
}
if(type=="6COL" || type=="7COL" || type=="8COL" )
{
           fileName = fileName                                        + sep +
                      "th6_"  + th6_str  + "eV";
}
if(type=="7COL" || type=="8COL" )
{
           fileName = fileName                                        + sep +
                      "th7_"  + th7_str  + "eV";
}
...
//  pepiEventAction->SetDoseLimit(fMaxDose);
//}
...
void PepiController::SetMaxCurrent(G4double maxCurrent)
{
  ...
  PepiEventAction* pepiEventAction = (PepiEventAction*)(G4RunManager::GetRunManager()->GetUserEventAction());
  pepiEventAction->SetCurrentLimit(fMaxCurrent);
}
...
void PepiController::SetMaxPixCount(G4double maxPixCount)
{
  ...
  PepiEventAction* pepiEventAction = (PepiEventAction*)(G4RunManager::GetRunManager()->GetUserEventAction());
  pepiEventAction->SetAvgPixLimit(fMaxPixCount);
}
