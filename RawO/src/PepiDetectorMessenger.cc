...
// new for PEPI
  ...
  fThreshold3Cmd = new G4UIcmdWithADoubleAndUnit("/Pepi/det/setThreshold3",this);
  fThreshold3Cmd->SetGuidance("Set detector's energy threshold 3. Default: 3 keV");
  fThreshold3Cmd->SetParameterName("setThreshold3",false);
  fThreshold3Cmd->SetUnitCategory("Energy");
  fThreshold3Cmd->AvailableForStates(G4State_PreInit);

  fThreshold4Cmd = new G4UIcmdWithADoubleAndUnit("/Pepi/det/setThreshold4",this);
  fThreshold4Cmd->SetGuidance("Set detector's energy threshold 4. Default: 3 keV");
  fThreshold4Cmd->SetParameterName("setThreshold4",false);
  fThreshold4Cmd->SetUnitCategory("Energy");
  fThreshold4Cmd->AvailableForStates(G4State_PreInit);

  fThreshold5Cmd = new G4UIcmdWithADoubleAndUnit("/Pepi/det/setThreshold5",this);
  fThreshold5Cmd->SetGuidance("Set detector's energy threshold 5. Default: 3 keV");
  fThreshold5Cmd->SetParameterName("setThreshold5",false);
  fThreshold5Cmd->SetUnitCategory("Energy");
  fThreshold5Cmd->AvailableForStates(G4State_PreInit);

  fThreshold6Cmd = new G4UIcmdWithADoubleAndUnit("/Pepi/det/setThreshold6",this);
  fThreshold6Cmd->SetGuidance("Set detector's energy threshold 6. Default: 3 keV");
  fThreshold6Cmd->SetParameterName("setThreshold6",false);
  fThreshold6Cmd->SetUnitCategory("Energy");
  fThreshold6Cmd->AvailableForStates(G4State_PreInit);

  fThreshold7Cmd = new G4UIcmdWithADoubleAndUnit("/Pepi/det/setThreshold7",this);
  fThreshold7Cmd->SetGuidance("Set detector's energy threshold 7. Default: 3 keV");
  fThreshold7Cmd->SetParameterName("setThreshold7",false);
  fThreshold7Cmd->SetUnitCategory("Energy");
  fThreshold7Cmd->AvailableForStates(G4State_PreInit);

  fThreshold8Cmd = new G4UIcmdWithADoubleAndUnit("/Pepi/det/setThreshold8",this);
  fThreshold8Cmd->SetGuidance("Set detector's energy threshold 8. Default: 3 keV");
  fThreshold8Cmd->SetParameterName("setThreshold8",false);
  fThreshold8Cmd->SetUnitCategory("Energy");
  fThreshold8Cmd->AvailableForStates(G4State_PreInit);
...
PepiDetectorMessenger::~PepiDetectorMessenger()
{
  ...
  delete fThreshold3Cmd;
  delete fThreshold4Cmd;
  delete fThreshold5Cmd;
  delete fThreshold6Cmd;
  delete fThreshold7Cmd;
  delete fThreshold8Cmd;
...
void PepiDetectorMessenger::SetNewValue(G4UIcommand* command,G4String newValue)
{
  ...
  {
    fDetectorConstruction->SetThreshold(fThreshold1Cmd->GetNewDoubleValue(newValue), fDetectorConstruction->GetThreshold2(), fDetectorConstruction->GetThreshold3(), fDetectorConstruction->GetThreshold4(), fDetectorConstruction->GetThreshold5(), fDetectorConstruction->GetThreshold6(), fDetectorConstruction->GetThreshold7(), fDetectorConstruction->GetThreshold8());
  }
  else if (command == fThreshold2Cmd)
  {
    fDetectorConstruction->SetThreshold(fDetectorConstruction->GetThreshold1(), fThreshold2Cmd->GetNewDoubleValue(newValue), fDetectorConstruction->GetThreshold3(), fDetectorConstruction->GetThreshold4(), fDetectorConstruction->GetThreshold5(), fDetectorConstruction->GetThreshold6(), fDetectorConstruction->GetThreshold7(), fDetectorConstruction->GetThreshold8());
  }
  else if (command == fThreshold3Cmd)
  {
    fDetectorConstruction->SetThreshold(fDetectorConstruction->GetThreshold1(), fDetectorConstruction->GetThreshold2(), fThreshold3Cmd->GetNewDoubleValue(newValue), fDetectorConstruction->GetThreshold4(), fDetectorConstruction->GetThreshold5(), fDetectorConstruction->GetThreshold6(), fDetectorConstruction->GetThreshold7(), fDetectorConstruction->GetThreshold8());
  }
  else if (command == fThreshold4Cmd)
  {
    fDetectorConstruction->SetThreshold(fDetectorConstruction->GetThreshold1(), fDetectorConstruction->GetThreshold2(), fDetectorConstruction->GetThreshold3(), fThreshold4Cmd->GetNewDoubleValue(newValue), fDetectorConstruction->GetThreshold5(), fDetectorConstruction->GetThreshold6(), fDetectorConstruction->GetThreshold7(), fDetectorConstruction->GetThreshold8());
  }
  else if (command == fThreshold5Cmd)
  {
    fDetectorConstruction->SetThreshold(fDetectorConstruction->GetThreshold1(), fDetectorConstruction->GetThreshold2(), fDetectorConstruction->GetThreshold3(), fDetectorConstruction->GetThreshold4(), fThreshold5Cmd->GetNewDoubleValue(newValue), fDetectorConstruction->GetThreshold6(), fDetectorConstruction->GetThreshold7(), fDetectorConstruction->GetThreshold8());
  }
  else if (command == fThreshold6Cmd)
  {
    fDetectorConstruction->SetThreshold(fDetectorConstruction->GetThreshold1(), fDetectorConstruction->GetThreshold2(), fDetectorConstruction->GetThreshold3(), fDetectorConstruction->GetThreshold4(), fDetectorConstruction->GetThreshold5(), fThreshold6Cmd->GetNewDoubleValue(newValue), fDetectorConstruction->GetThreshold7(), fDetectorConstruction->GetThreshold8());
  }
  else if (command == fThreshold7Cmd)
  {
    fDetectorConstruction->SetThreshold(fDetectorConstruction->GetThreshold1(), fDetectorConstruction->GetThreshold2(), fDetectorConstruction->GetThreshold3(), fDetectorConstruction->GetThreshold4(), fDetectorConstruction->GetThreshold5(), fDetectorConstruction->GetThreshold6(), fThreshold7Cmd->GetNewDoubleValue(newValue), fDetectorConstruction->GetThreshold8());
  }
  else if (command == fThreshold8Cmd)
  {
    fDetectorConstruction->SetThreshold(fDetectorConstruction->GetThreshold1(), fDetectorConstruction->GetThreshold2(), fDetectorConstruction->GetThreshold3(), fDetectorConstruction->GetThreshold4(), fDetectorConstruction->GetThreshold5(), fDetectorConstruction->GetThreshold6(), fDetectorConstruction->GetThreshold7(), fThreshold8Cmd->GetNewDoubleValue(newValue));
  }
...
G4String PepiDetectorMessenger::GetCurrentValue(G4UIcommand* command)
{
  ...
  {
    answer = fThreshold3Cmd->ConvertToString(fDetectorConstruction->GetThreshold3());
  }
  else if(command == fThreshold4Cmd)
  {
    answer = fThreshold4Cmd->ConvertToString(fDetectorConstruction->GetThreshold4());
  }
  else if(command == fThreshold5Cmd)
  {
    answer = fThreshold5Cmd->ConvertToString(fDetectorConstruction->GetThreshold5());
  }
  else if(command == fThreshold6Cmd)
  {
    answer = fThreshold6Cmd->ConvertToString(fDetectorConstruction->GetThreshold6());
  }
  else if(command == fThreshold7Cmd)
  {
    answer = fThreshold7Cmd->ConvertToString(fDetectorConstruction->GetThreshold7());
  }
  else if(command == fThreshold8Cmd)
  {
    answer = fThreshold8Cmd->ConvertToString(fDetectorConstruction->GetThreshold8());
  }
